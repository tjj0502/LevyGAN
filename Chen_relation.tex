...
\[
A^{i,j}_{0,t} = A^{i,j}_{0,s} + A^{i,j}_{s,t} + \frac{1}{2}\left( W^i_{0,s} W^j_{s,t} - W^j_{0,s} W^i_{s,t} \right)
\]
for \( 0 \leq s \leq t \).
...
\[
A^{i,j}_{s,t} = H^i_{s,t} W^j_{s,t} - W^i_{s,t} H^j_{s,t} + b^{i,j}_{s,t}
\]

\[
b^{i,j}_{s,t} \coloneqq \int_s^t B^i_{s,u} \circ dB_u^j
\]

\[
\tilde{A}^{i,j}_{s,t} = c_i H^i_{s,t} W^j_{s,t} - c_j W^i_{s,t} H^j_{s,t} + c_i c_j \tilde{b}^{i,j}_{s,t} \; \; \text{   for  } \vc \in \{-1, 1 \}^d
\]
